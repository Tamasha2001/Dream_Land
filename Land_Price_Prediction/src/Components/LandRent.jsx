...
const LandRent = () => {
  ...
  const fetchLands = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/land_rent/getAllLands');
  ...
  const handleSaveLand = async (e) => {
    ...
    try {
      if (editLandId) {
        // Update land
        await axios.put(`http://localhost:8080/api/land_rent/updateLand/${editLandId}`, formData, {
          headers: {
        ...
        // Add new land
        await axios.post('http://localhost:8080/api/land_rent/saveLand', formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:8080/api/land_rent/deleteLand/${id}`);
  ...
  const handleSearchLand = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/land_rent/searchLand/${searchId}`);
      if (response.data) {
